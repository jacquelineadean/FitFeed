const cellCount = 3; //this should evenly go into the recipe count for now
let recipes = [];
let recipeURL = '';

$(document).ready(function () {
    $.get("https://api.spoonacular.com/recipes/random?apiKey=6983116be6a04b6d8b5f03725e5b859e&number=6&targetCalories=1000", function (data, status) {
        //alert("Data: " + data + "\nStatus: " + status);
    ...
    $(document).on("click",".show-recipe", function(e){
        ...
        const recipeId = e && e.currentTarget && e.currentTarget.id;
        showRecipe(recipeId);
    });

    $('#recipeWindow').on('shown.bs.modal',function() {      //correct here use 'shown.bs.modal' event which comes in bootstrap3
...
function displayRecipeImages() {
    if (!recipes || recipes.length === 0) { //we have no recipes, nothing to display
        return;
    }
    $('#recipe-images').html(''); //empty the div
    const collection = chunkArray(recipes, cellCount);
    collection.forEach(smallArray => {
        //add a row
        const div = $("<div>", {"class": "row my-4"});
        //build our recipe images display
        const recipeImages = smallArray.map(x => { //loop through the array of recipes and build an array of images
